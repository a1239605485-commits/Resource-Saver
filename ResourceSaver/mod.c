...
static void init_mod(kernel_mod_handle_t* handle) {
    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ResourceSaver",
        "Resource Saver v1.2.7 initializing (v1.1.0 core, safe Chinese MouseText settings UI, author: liuxin)"
    );
    ...
    resource_saver_settings_ui_init(handle ? handle->private_dir : NULL);

    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ResourceSaver",
        "Resource Saver v1.2.7 initialized"
    );
}
...
static void cleanup_mod(kernel_mod_handle_t* handle) {
    ...
    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ResourceSaver",
        "Resource Saver v1.2.7 unloaded"
    );
}

static kernel_mod_info_t g_info = {
    .pkg_id = "celso.resourcesaver",
    .version_code = 202608284,
    .api_version = 1,
    .version = "1.2.7"
};
