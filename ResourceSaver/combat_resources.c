...
/*
 * Resource Saver v1.2.1 - v1.1.0 combat core + independent switches
 *
...
static void reset_effects_postfix(
    ...
    if ((g_player_ammo_cost80 || g_player_huntress_ammo_cost90) &&
        resource_saver_feature_enabled(RS_FEATURE_MASTER)) {
        bool enabled = true;
        bool regular_on = resource_saver_feature_enabled(RS_FEATURE_REGULAR_AMMO);
        bool special_on = resource_saver_feature_enabled(RS_FEATURE_SPECIAL_AMMO);

        if (!state->held_class_valid) {
            /*
             * 保留 v1.1.0 的稳定回退：只有普通/特殊两个弹药开关都开启时，
             * 无法分类才使用已验证的 ammoCost80 20% 回退。
             * 如果用户只关掉其中一个，则不做错误分类。
             */
            if (regular_on && special_on && g_player_ammo_cost80) {
            ...
            if (use_ammo == RS_AMMO_NONE || is_tool_ammo_group(use_ammo)) {
                ...
                if (regular_on && g_player_ammo_cost80) {
                    patchlib_field_set_value(g_player_ammo_cost80, instance, &enabled);
                    ++g_stat_ammo20_frames;
                }
            }
            else if (is_special_ammo_group(use_ammo)) {
                if (special_on && g_player_huntress_ammo_cost90) {
                    patchlib_field_set_value(g_player_huntress_ammo_cost90, instance, &enabled);
                    ++g_stat_ammo10_frames;
                }
                else if (special_on && g_player_ammo_cost80 && !g_player_huntress_ammo_cost90) {
                    /* 10%字段不存在时才回退到20%。 */
                    patchlib_field_set_value(g_player_ammo_cost80, instance, &enabled);
                    ++g_stat_ammo20_frames;
                }
     ...
     */
    if (g_player_mana_cost && resource_saver_feature_enabled(RS_FEATURE_MASTER)) {
        float multiplier = 1.0f;

        if (state->held_class_valid && state->held_is_summon) {
            if (resource_saver_feature_enabled(RS_FEATURE_SUMMON_MANA)) {
                multiplier = 0.75f;
            ...
            /*
             * 完整保留 v1.1.0 已验证有效的 Player.manaCost x 0.85 实现。
             * HeldItem 分类失败时也仍按 v1.1.0 的基础 -15% 回退。
             */
            if (resource_saver_feature_enabled(RS_FEATURE_MAGIC_MANA)) {
                multiplier = 0.85f;
...
static void update_postfix(
    ...
    if (g_player_mana_regen && g_player_mana_regen_count &&
        resource_saver_feature_enabled(RS_FEATURE_MANA_REGEN)) {
        int mana_regen = 0;
    ...
    if (g_player_potion_delay &&
        resource_saver_feature_enabled(RS_FEATURE_MELEE_POTION)) {
        int potion_delay = 0;
...
void resource_saver_combat_init(void) {
    ...
    if (!player_type || !item_type) {
        mod_logger_write(
            MOD_LOG_LEVEL_ERROR,
            "ResourceSaver",
            "Combat v1.2.1 init failed: Player=%p Item=%p",
            player_type,
            item_type
        );
        goto done;
    ...
    mod_logger_write(
        MOD_LOG_LEVEL_INFO,
        "ResourceSaver",
        "Combat v1.2.1 (v1.1.0 core) hooks: ResetEffects=%d Update=%d ammo20=%p ammo10=%p manaCost=%p HeldItem=%p useAmmo=%p melee=%p summon=%p sentry=%p",
